...
class Strategy extends require("./strategy.coffee")
	...
	execute: =>
		process.stdout.write "Starting #{@app_name}...\n".yellow
		if @container_id
		...
		if @container_id
			@ids = [@container_id]
			f = Q.denodeify(@_docker_start_stop_restart)("start", @container_id, @opts).then =>
			...
			f = Q.denodeify(@_docker_get_ids_from_appname)(@app_name).then (ids) =>
				...
				cmd = (index) =>
					...
					@_docker_start_stop_restart "start", ids[index], @opts, (err) =>
						process.stdout.write "...done!\n".green
