...
class Strategy extends require("./strategy.coffee")
	...
	execute: =>
		process.stdout.write "Restarting #{@app_name}...\n".yellow
		if @container_id
		...
		if @container_id
			@ids = [@container_id]
			f = Q.denodeify(@_docker_start_stop_restart)("restart", @container_id, @opts).then =>
			...
			f = Q.denodeify(@_docker_get_ids_from_appname)(@app_name).then (ids) =>
				...
				cmd = (index) =>
					...
					@_docker_start_stop_restart "restart", ids[index], @opts, (err) =>
						process.stdout.write "...done!\n".green
